...
{
    public class MyFilteringAdapter
        : MyAdapter, IFilterable
    {
        private class MyFilter : Filter
        {
            private readonly MyFilteringAdapter _owner;

            public MyFilter(MyFilteringAdapter owner)
            {
                _owner = owner;
            }
        ...
        public MyFilteringAdapter(Context context) : base(context)
        {
            ReturnSingleObjectFromGetItem = true;
            Filter = new MyFilter(this);
        }

        protected MyFilteringAdapter(IntPtr javaReference, JniHandleOwnership transfer)
